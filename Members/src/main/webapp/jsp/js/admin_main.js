$(document).ready(function(){
	ajaxs("main/f_user_main/getNewMembs.do","get",{},true,function(data) {
		if(data) {
			var htm = '';
			for(var i in data) {
				htm += '<tr>';
				...
				htm += '<td class="hidden-480">' + (data[i].mRecommderName ? data[i].mRecommderName : "") + '</td>';
				htm += '<td class="hidden-480">' + data[i].mRegdate + '</td>';
				htm += '</tr>';
			}
			$("#recnetMembs").html(htm);
		}
	},"json");
	ajaxs("main/f_user_main/getNewRemit.do","get",{},true,function(data) {
		if(data) {
			var htm = '';
			for(var i in data) {
				htm += '<tr>';
				htm += '<td class="hidden-480">' + data[i].extendPro.membAccount + '</td>';
				htm += '<td>' + data[i].extendPro.membName + '</td>';
				htm += '<td>' + data[i].rMoney + '</td>';
				htm += '<td>' + data[i].rBAccount + '</td>';
				htm += '<td class="hidden-480">' + data[i].rSubDate + '</td>';
				htm += '<td class="hidden-480">' + data[i].rRemark + '</td>';
				htm += '</tr>';
			}
			$("#recentRemits").html(htm);
		}
	},"json");
});
